...
|#
(define (n-copies s n)
  (if (> n 0) (string-append (n-copies s (- n 1)) s) "" ))
...
(define (num-evens numbers)
  ...
  (if (= (length numbers) 0 )
      0
      (if (= (modulo (first numbers) 2) 0)
          (+ 1 (num-evens (rest numbers)))
          (num-evens (rest numbers)))
  ...
  (cond
    ...
    [else
     ...
     (let ([first-number (first numbers)]
           [NAME-ME-BETTER (num-evens (rest numbers))])  ; Pick a better variable name.
       ...)])
  |#
  )
...
(define (num-many-evens lists-of-numbers)
   ...
   (if (= (length lists-of-numbers) 0 ) 0
       (if (= (length lists-of-numbers) 1)
           ; true 
           (if  (< 2 (num-evens (first lists-of-numbers) )) 1 0)
           ; false
           (if  (< 2 (num-evens (first lists-of-numbers) ))
                
                (+ 1 (num-many-evens (rest lists-of-numbers)))
                (num-many-evens (rest lists-of-numbers))
...
(define (calculate expr)
  (if (list? expr)
      ((calculate (list-ref expr 0) ) (calculate (list-ref expr 1) ) (calculate (list-ref expr 2) ))
      (if (number? expr)
          expr
          (if (eq? expr '+) +
              (if (eq? expr '-)-
